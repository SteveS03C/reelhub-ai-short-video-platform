		...
		columns : [
			...
			},
			{
				...
				formatter : function(value, row, index) {
					...
					// 查看
					var a = '<a  class=" btn-sm btn-primary" onclick="showVideoDetail(\'' + row.id + '\')" data-target="#myModal">查看</a> ';
					// 删除
...
function showVideoDetail(videoId) {
	$.ajax({
		...
		success : function(data) {
			if (data.status == 200) {
				var videoInfo = data.data;
				$('.news_title').text(videoInfo.videoDesc);
				$('.news_intr').text(videoInfo.videoDesc);
				var videoHtml = '<video src="' + videoInfo.videoPath + '" controls="controls" style="width:100%;height:300px;"></video>';
				$('.news_infos').html(videoHtml);
				$('#myModal').modal('show');
			} else {
				swal("获取视频详情失败", "请重新操作", "error");
			}
		},
		error : function() {
			swal("获取视频详情错误", "请重新操作", "error");
		}
	});
}
