...
var selectReport = function() {
	$('#allReport').bootstrapTable('destroy');
		...
		columns : [
			...
			},
			{
				...
				formatter : function(value, row) {
					var status = row.processStatus || 'PENDING';
					if (status === 'DONE') {
			...
			},
			{
				...
				width : '12%',
				formatter : function(value, row) {
					return '<span>' + (row.processResult || '未处理') + '</span>';
				}
			},
			{
				title : '处理人',
				...
				width : '8%',
				formatter : function(value, row) {
					return '<span>' + (row.processAdmin || '-') + '</span>';
				}
			},
			{
				title : '处理时间',
				...
				width : '12%',
				formatter : function(value, row) {
					return row.processTime ? Format(row.processTime, 'yyyy-MM-dd hh:mm') : '-';
				}
			},
			{
				title : '執行',
				...
				formatter : function(value, row, index) {
					if (row.status == 2) {
						return '<a class="btn-sm btn-primary" onclick="operation(\'' + row.id + '\',\'' + row.dealVideoId + '\',\'上架\')"><i class="fa fa-share-square-o"></i> 恢复上架</a>';
					} else if (row.status == 1) {
						return '<a class="btn-sm btn-danger" onclick="operation(\'' + row.id + '\',\'' + row.dealVideoId + '\',\'下架\')"><i class="fa fa-share-square-o"></i> 执行下架</a>';
					}
					return '<span class="text-muted">无可用操作</span>';
				},
...
function operation(reportId, id, op)
{
	  var status=0;
      ...
      $.ajax({
    	    		 url:"../../report/undercarriage?reportId="+reportId+"&id="+id+"&status="+status,
    	    		 method : 'post',
